...
import { defined, entries, fromEntries, full, keysOf, Pair, pairs, readLines } from "../../utils";
...
const runFactory = (factory: Factory) => {
    ...
    return nextBuild({ factory, builds: [], geodes: 0 }, 24);
}

const maxes = factories.map(runFactory);
const quality = maxes.reduce((acc, max, i) => acc + (max * (i + 1)), 0);
